...
const { marketKey, rolloverDelay } = require('../../lib/market-rollover');
...
class MasterAgent extends EventEmitter {
  constructor(config) {
    ...
    this._marketRefreshInterval = null;
    this._marketRolloverTimeout = null;
  ...
  async start({ tradingEnabled = false } = {}) {
    if (this.running) {
      if (tradingEnabled) this.enableTrading();
      return this.state;
    }
    ...
    // Start read-only monitoring immediately. The dashboard must not wait for
    // a Binance tick before it can refresh Kalshi markets and balance data.
    this._marketRefreshInterval = setInterval(() => this._refreshMarketsOnly(), 2000);
    ...
    this._scheduleMarketRollover();

    if (tradingEnabled) {
      stateManager.botState.updateIntent({
        status: 'waiting',
        message: 'Waiting for Binance price feed before trading...',
      });
      ...
      stateManager.botState.updateIntent({
        status: 'monitoring',
        message: 'Market data is live. Trading is paused.',
        action: null,
      });
      // This is informational only. Monitoring is already running above.
      this._waitForPrice().catch(() => {});
    }

    this.log(tradingEnabled ? 'Engine running. Trading enabled.' : 'Engine running. Monitoring only.');
  ...
  enableTrading() {
    ...
    orderExecutor.startOrderManager();
    this._scanInterval = setInterval(() => this._runScan(), 2000);
    this._takeProfitInterval = setInterval(() => this._runTakeProfit(), 3000);
    this.tradingEnabled = true;
    this.state.updateIntent({ status: 'scanning', message: 'Scanning for opportunities...' });
    this.log('Trading enabled. Order manager and scan loops started.');
  }
  ...
  disableTrading() {
    ...
    this._scanInterval = null;
    this._takeProfitInterval = null;
    this.registry.get('order-executor').stopOrderManager();
    this.state.updateIntent({
      status: 'monitoring',
      message: 'Market data is live. Trading is paused.',
      action: null,
    });
    this.log('Trading paused. Market monitoring remains active.');
  }
   ...
   */
  async _runScan() {
    if (this._scanRunning || !this.running || !this.tradingEnabled) return;
    ...
    try {
      ...
      if (signals.length > 0) {
        ...
        const now = Date.now();
        const currentMarkets = state.activeMarkets.filter((market) => (
          market.openTime <= now
          && market.closeTime > now
          && market.status !== 'closed'
        ));
        const quotedMarkets = currentMarkets.filter((market) => (
          Number.isFinite(market.yesAsk)
          && Number.isFinite(market.noAsk)
          && market.yesAsk > 0 && market.yesAsk < 1
          && market.noAsk > 0 && market.noAsk < 1
        ));

        state.updateIntent({
          status: quotedMarkets.length > 0 ? 'scanning' : 'waiting_for_quotes',
          message: quotedMarkets.length > 0
            ? 'Scanning for opportunities...'
            : currentMarkets.length > 0
              ? 'Waiting for valid two-sided Kalshi Demo quotes...'
              : 'Waiting for the next open KXBTC15M contract...',
          currentEdge: null,
          action: null,
        });
      }
  ...
  async _runDiscovery() {
    ...
    this._discoveryPromise = (async () => {
      try {
        const beforeKey = marketKey(this.state?.activeMarkets?.[0]);
        const result = await this.orchestrator.dispatch({ action: 'discover-markets', params: {} });
        const afterKey = marketKey(this.state?.activeMarkets?.[0]);
        if (result.success && result.count > 0 && afterKey !== beforeKey) {
  ...
  /**
   * Align discovery to the exact contract close instead of waiting up to
   * 15 seconds for the periodic fallback. If Kalshi has not published the
   * next contract yet, rolloverDelay() retries every 500 ms.
   */
  _scheduleMarketRollover() {
    if (this._marketRolloverTimeout) clearTimeout(this._marketRolloverTimeout);
    this._marketRolloverTimeout = null;
    if (!this.running) return;

    const delay = rolloverDelay(this.state?.activeMarkets, Date.now());
    this._marketRolloverTimeout = setTimeout(() => {
      this._marketRolloverTimeout = null;
      this._runDiscovery();
    }, delay);
    this._marketRolloverTimeout.unref?.();
  }

  async refreshMarketsNow() {
    return this._runDiscovery();
  }

  async _refreshMarketsOnly() {
    if (!this.running) return;
    try {
      await this.orchestrator.dispatch({ action: 'refresh-markets', params: {} });
    } catch (err) {
      this.log(`Market refresh error: ${err.message}`, 'WARN');
    }
  }
  ...
  stop() {
    ...
    if (this._marketRefreshInterval) clearInterval(this._marketRefreshInterval);
    if (this._marketRolloverTimeout) clearTimeout(this._marketRolloverTimeout);
    this._marketRolloverTimeout = null;
