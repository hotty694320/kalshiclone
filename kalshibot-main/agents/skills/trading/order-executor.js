...
class OrderExecutor extends BaseSkill {
  constructor() {
    ...
    this.failedTickerAttempts = new Map();
    this.failedRetryMs = 15000;
  }
    ...
    switch (task.action) {
      case 'execute-signals': {
        ...
        for (const signal of signals) {
          ...
          const lastFailure = this.failedTickerAttempts.get(signal.ticker);
          if (lastFailure && Date.now() - lastFailure < this.failedRetryMs) {
            results.push({ signal: signal.ticker, status: 'blocked', reason: 'order_retry_cooldown' });
            continue;
          }
  ...
  async _executeSignal(signal, state, kalshiSkill, analyticsSkill) {
    ...
    try {
      ...
      this.failedTickerAttempts.delete(signal.ticker);
      ...
      const detail = err.response
        ...
        : err.message;
      this.failedTickerAttempts.set(signal.ticker, Date.now());
      console.error(`[OrderExecutor] Order failed for ${signal.ticker}: ${detail}`);
      state.updateIntent({
        status: 'order_error',
        message: `Kalshi rejected ${signal.side.toUpperCase()} order: ${detail}`,
        action: null,
      });
