...
class BotState extends EventEmitter {
  constructor() {
    ...
    // Read-only forecast state is separate from trading strategy state.
    this.prediction = {
      marketTicker: null,
      status: 'unavailable',
      finalizeAt: null,
      finalizedAt: null,
      independent: { up: null, down: null },
      marketAssisted: { up: null, down: null },
      dataHealth: 'unavailable',
      reason: 'Waiting for an active KXBTC15M market.',
  ...
  updateKalshiNowPrice(price) {
    this.btcPrice.kalshiNow = price;
    this.btcPrice.kalshiNowUpdate = Date.now();
    this.emit('price:kalshi-now', { price, timestamp: this.btcPrice.kalshiNowUpdate });
  }
  ...
  updatePrediction(prediction) {
    this.prediction = {
      ...
      marketAssisted: {
        ...this.prediction.marketAssisted,
        ...(prediction?.marketAssisted || {}),
      },
    };
